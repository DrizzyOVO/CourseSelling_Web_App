...
import { courseState, courseInterface } from "@repo/store/src/atoms/course";
import {useRecoilState, useRecoilValue, useSetRecoilState} from "recoil";
import { courseTitle, coursePrice, isCourseLoading, courseDescription } from "@repo/store/src/selectors/course";
...
function Course({ params } : {params : any}) {
    ...
    useEffect(() => {
      async function smth() {
          ...
          const response = await axios.get(`http://localhost:5000/user/courses/${courseId}`, { 
              headers: {
    ...
    const buyCourse = async () => {
      ...
      const response = await axios.get(`http://localhost:5000/user/courses/${courseId}/buy`, {
        headers: {
          authorization: `Bearer ${localStorage.getItem("token")}`
        }
      }); 
      ...
      if(response.data.message == "Bought it") {  
        setBought(true); 
        router.push(`http://localhost:3000/courses/${courseId}/purchased`); 
      }
